...
for (i in 1:length(test_lines)) {
  for (j in 1:runs) {
    ...
    command_lines = c(command_lines, paste(exe, exe_args, param_fijos, instancia, test_lines[i], "--seed", semillas[j]))
  }
}

# Ejecutar los experimentos
cat("\nEjecutando ", length(command_lines), " experimentos...\n")
all_output = mclapply(command_lines, runExperiment, mc.cores = cores, command="java")
