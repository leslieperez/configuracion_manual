...
exe_args = "--tries 1 --time 0 --tours 1000 --quiet"

# Defina parametros fijos
param_fixed = "--ants 10 --localsearch 0"
...
for (i in 1:length(test_lines)) {
  ...
  for (j in 1:runs) {
    ...
    command_line = paste(exe, exe_args, param_fixed, instancia, test_lines[i], "--seed", semillas[j])
    resultados[j,i] = runExperiment(command="java", command_line = command_line)
